...
const DirectCOAttainmentReport = () => {
  ...
  const fetchAttainmentData = async () => {
    ...
    try {
      const [cie1Res, cie2Res, seeRes] = await Promise.all([
        axios.get(
          `${process.env.REACT_APP_BACKEND_URI}/api/attainment/subject/${selectedSubject}/examType/CIE-1`
        ),
        axios.get(
          `${process.env.REACT_APP_BACKEND_URI}/api/attainment/subject/${selectedSubject}/examType/CIE-2`
        ),
        axios.get(
  ...
  const calculateAverageAttainment = (coNo) => {
    ...
    if (cie1Data?.attainmentData) {
      const cie1Entry = cie1Data.attainmentData.find(
        (item) => item.coNo === coNo
      );
      if (cie1Entry) {
    ...
    if (cie2Data?.attainmentData) {
      const cie2Entry = cie2Data.attainmentData.find(
        (item) => item.coNo === coNo
      );
      if (cie2Entry) {
  ...
  const getExamTypes = (coNo) => {
    const exams = [];
    if (cie1Data?.attainmentData.some((item) => item.coNo === coNo))
      exams.push("CIE-1");
    if (cie2Data?.attainmentData.some((item) => item.coNo === coNo))
      exams.push("CIE-2");
    return exams.join(", ");
  };

  const allCOs = showTable
    ? [
        ...new Set([
  ...
  const postAttainmentData = async () => {
    ...
    if (showTable && allCOs.length > 0) {
      try {
        ...
        const mappedAttainmentData = allCOs
          .map((coNo) => ({
            coNo,
            attainmentLevel: parseFloat(
              calculateDirectCoAttainment(
                calculateAverageAttainment(coNo),
                seeData?.attainmentData.find((item) => item.coNo === coNo)
                  ?.attainmentLevel || "-"
              )
            ),
          }))
  ...
  const exportToPDF = async () => {
    ...
    const contentOriginal = document.querySelector(
      ".flex.flex-col.gap-6.w-full.max-w-5xl"
    );
    ...
    const graphContainerElement = graphWrapper.querySelector(
      ".flex.justify-center"
    );
