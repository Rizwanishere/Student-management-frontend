...
function PostSubjects (){
      ...
      reader.onload = (e)=>{
        try{
          ...
          for(let i = headerRowIndex+1; i<rawRows.length;i++){
            ...
            const semester = semesterIndex !== -1 ? String(row[semesterIndex] || "").trim() : "";
            const regulation = regulationIndex !== -1 ? String(row[regulationIndex] || "").trim() : "";
            const courseCode = courseCodeIndex !== -1 ? String(row[courseCodeIndex] || "").trim() : "";
  ...
  const validateExcelData = (data) => {
    ...
    data.forEach((row,index)=>{
      ...
      if(!name){
        ...
        errors.push(`Row ${index+1}: Duplicate Subject Name "${name}" - must be unique`);
      } else {
        nameSet.add(name);
      }
  ...
  const handleExcelImport = async(e)=>{
    ...
    try{
      ...
      for(const importedSubject  of subjectsToImport){// TODO: Change the existing Subject condition
        const existingSubject = subjects.find((subject)=> subject.name.toLowerCase() === importedSubject.name.toLowerCase());
        if(existingSubject){
    ...
    try {
      ...
      for(const record of subjects){
        try{
          ...
          const response = await fetch(`${process.env.REACT_APP_BACKEND_URI}/api/subjects`,{
            method: "POST",
            headers: {
